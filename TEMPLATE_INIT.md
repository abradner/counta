...
10. **Caveman mode**: keep the terse-tone module or prune it? (It's a tone preference, not a
...
11. **Skills**: keep both shipped skills? `batch-review` only earns its keep in repos that will
    ship multi-PR bodies of work; `independent-commit-review` is broadly useful. When in doubt,
    keep — they're inert until invoked.
...
- `.claude/skills/batch-review/SKILL.md`: fill the "Repo specifics" section (validation commands,
  bot roster + trigger behavior, merge strategy — keep only the matching variant of each
  strategy-flagged rule, delete the other).
- `mise.toml`: uncomment/fill the stack's tools; delete the rest.
- `.gitignore`: uncomment the relevant language block(s); delete the rest.

## Phase 3 — Prune (or wire in) modules

For each module the operator declined, delete its files entirely — no commented-out corpses.
