...
const [trancheResults, reverified] = await Promise.all([
  ...
  parallel(reverifyChunks.map(([a, b]) => () => agent([
    ...
    .then(r => {
      ...
      // chunk — see SKILL.md's known failure modes) — that's a different failure than a
      // well-formed response quietly missing an id, so the fallback evidence says which one.
      ...
      const fallbackEvidence = r
        ? 'agent omitted this id from its response — treated as unconfirmed, not silently dropped'
        : 'agent call for this chunk returned no response (schema-retry cap) — treated as unconfirmed, not silently dropped'
      for (let id = a; id <= b; id++) {
        if (!byId.has(id)) byId.set(id, { id, status: 'open', evidence: fallbackEvidence })
      }
