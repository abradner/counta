...
- **Numbers stop at the step boundary — but only the plan's number.** "Doses left" on a planned pen is doses left *at this amount*, and the tile names the amount, because forecasting past a step change would mean guessing what the next strength's pen holds. Everything that is a question about the *pen* rather than the plan — the calendar export, the expiry projection, the running-low warnings — keeps counting the whole pen at the size actually dialled. Collapsing the two silently truncated the calendar and made a full pen claim it had nothing left; they are separate functions in `app.js` now, with the distinction written above them.
