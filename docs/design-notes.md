...
- **Numbers stop at the step boundary — but only the plan's number.** "Doses left" on a planned pen is doses left *at this amount*, and the tile names the amount, because forecasting past a step change would mean guessing what the next strength's pen holds. Everything that is a question about the *pen* rather than the plan — the expiry projection, the running-low warnings — keeps counting the whole pen at the size actually dialled. Collapsing the two silently truncated the calendar and made a full pen claim it had nothing left; they are separate functions in `app.js` now, with the distinction written above them. *(This list named **the calendar export** too, until 2026-08-11. It no longer does — see the next bullet. Corrected in place rather than rewritten, per AGENTS.md §10 meta-rules 1–2.)*
- **The calendar export walks the ladder (#45, 2026-08-11).** The rule above was right that the plan's number must not leak into the pen's, and wrong about which question the calendar asks. Counting the whole pen *at the size actually dialled* assumes the user ignores their own plan: a Wegovy pen exported 37 reminders all reading "dial 8 clicks", when the ladder they had just transcribed changes the amount after four. Both failures — the truncation that fix was reacting to, and the flat repetition it produced — are avoided by walking the ladder against **this pen's** clicks-per-unit ratio and emitting one event per step. What made that safe was noticing the export never needed the thing #19 blocks: `calendarUid` is per-pen, so a series only ever describes the pen in front of the user, and the next pen mints its own. The same Wegovy pen now exports 4+4+4+1 = 13 doses and puts "buy more" 77 days out instead of 245. A step this pen cannot dial has no laddered answer, so the export falls back to the flat count rather than scheduling nothing.
- **Most people join partway up.** Someone transcribing a published escalation has usually been on it for weeks, so the card asks which amount they're taking now and how many they've already had at it — defaulting to "just starting", which costs a genuinely new user nothing. The other framing (what your *next* dose will be) was rejected: it makes the user work out whether they're due to step up, which is precisely the thing the ladder exists to work out for them. Answers from either side of a step boundary converge on the same position, and the app restates the result before saving, so the ±1 reading can't land anyone a dose out.
- **Prior doses belong to the plan, not to a pen.** They were often taken on a freebie starter pen, on tablets, or on a pen counta was never told about. Backdating them into some pen's dose log would record events that pen never saw, so they're a single count on the plan that travels with it onto every later pen — added once, never per pen. The plan's start date therefore means "when counta starts counting", not when the ramp began.
- **A plan is never offered between unlisted pens.** Every "Something else…" pen shares one product key, so donor matching on it would hand an unlisted insulin pen the milligram ladder from an unlisted GLP-1. Nothing records that two unlisted pens hold the same drug, so counta doesn't guess — the plan is transcribed again by hand.
