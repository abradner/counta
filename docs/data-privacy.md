...
Client-generated **ICS file** (download/share — not a hosted subscription URL, which would hand the schedule to a server): a recurring dose event for the remaining doses in the pen, plus a "buy more <product>" event ~2 doses before run-out. On a pen carrying a dose plan this is one recurring event *per ladder step* rather than one for the pen (#45), since RFC 5545 cannot vary an event's summary across occurrences of a single rule, plus `STATUS:CANCELLED` tombstones for steps the pen has left behind. Honest caveat in UI: your calendar provider then holds your schedule — that's the user's tradeoff to make.
...
UIDs are per-pen but not derived from the server row id: `calendarUid` (minted once), `calendarSequence` (incremented every export, giving RFC 5545 `SEQUENCE` so a client can't ignore an update) and `calendarSteps` (#45 — the high-water mark of how many per-step event UIDs this pen has ever written, so a later export can still cancel them once the plan naming them is shortened or removed) live inside the pen's own encrypted blob, alongside dose history. That way a pen recreated from a preserved blob keeps superseding its own old calendar events instead of leaving stale ones behind, without hashing batch/expiry into anything that lands in a third-party calendar.
